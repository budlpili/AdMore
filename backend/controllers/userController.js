...
const verifyEmail = (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE verifyToken = ?', [token], (err, user) => {
    ...
    db.run('UPDATE users SET emailVerified = 1, verifyToken = NULL, verifyExpires = NULL WHERE id = ?', [user.id], (updateErr) => {
      if (updateErr) {
...
const updateProfile = (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE id = ?', [userId], (err, user) => {
    ...
    db.run('UPDATE users SET name = ?, phone = ? WHERE id = ?', [name, phone, userId], (updateErr) => {
      if (updateErr) {
...
const changePassword = (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE id = ?', [userId], (err, user) => {
    ...
    bcrypt.compare(currentPassword, user.password, (compareErr, isMatch) => {
      if (compareErr) {
        console.error('비밀번호 확인 오류:', compareErr);
        return res.status(500).json({ message: '비밀번호 확인 중 오류가 발생했습니다.' });
      }
      ...
      bcrypt.hash(newPassword, 10, (hashErr, hashedPassword) => {
        if (hashErr) {
          console.error('비밀번호 해시화 오류:', hashErr);
...
const resendVerifyEmail = (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE email = ?', [email], (err, user) => {
    ...
    db.run('UPDATE users SET verifyToken = ?, verifyExpires = ? WHERE id = ?', [verifyToken, verifyExpires, user.id], (updateErr) => {
      if (updateErr) {
        console.error('토큰 업데이트 오류:', updateErr);
        return res.status(500).json({ message: '토큰 재발송에 실패했습니다.' });
      }
